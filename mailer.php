...
if (isset($_FILES["userfile-1"]["tmp_name"])) {
    ...
    $projectType = trim($_POST["tipo-proyecto"]);
    $projectSize = trim($_POST["tanano-proyecto"]);
    $projectLocation = trim($_POST["ubicacion-proyecto"]);
    ...
    try {
        ...
        if (isset($_FILES["userfile-1"]["tmp_name"])) {
            foreach ($_FILES["userfile-1"]["name"] as $k => $v) {
                $mail->AddAttachment(
                    $_FILES["userfile-1"]["tmp_name"][$k],
                    $_FILES["userfile-1"]["name"][$k]
                );
            }
        }
        ...
        $mail->Body =
            ...
            "<br>Tipo de proyecto: " .
            $projectType .
            "<br>Tamaño del proyecto: " .
            $projectSize .
            "<br>Ubicación del proyecto: " .
            $projectLocation .
            "<br>Empresa: " .
            $company .
            "<br><br>Mensaje: " .
            $message .
            "<br><br>Este mensaje fue enviado a través del formulario de contacto del sitio de Comsa Steel.";
